...
require_once __DIR__ . '/SignalImageGenerator.php';
...
class SignalEngine
{
    ...
    private SignalImageGenerator $imageGen;
    ...
    public function __construct(OpenRouter $ai, MarketData $market, Telegram $telegram, Logger $logger)
    {
        ...
        $this->logger   = $logger;
        $this->imageGen = new SignalImageGenerator($logger);
    ...
    public function run(): array
    {
        ...
        try {
            ...
            if ($signal['signal'] !== 'NO_SIGNAL' && $signal['confidence'] !== 'Low') {
                if ($this->channel) {
                    // تولید تصویر
                    $imagePath = $this->imageGen->generate($signal, $priceData);
                    
                    if ($imagePath) {
                        // ارسال تصویر + کپشن
                        $sent = $this->telegram->sendPhoto($this->channel, $imagePath, $message);
                        @unlink($imagePath);
                    } else {
                        // فالبک به پیام متنی
